...
def labelling (fileName):
  ...
  for image in fileName:
    ...
    labels.append(float(image[13:17]))
...
def show_output(y_test, yTest, fname, heading):
...
show_output(y_test, yTest, main_op, main_heading)
...
for ind, act in enumerate(y_test):
  ...
  if noise >= 0.05 and noise <= 0.3:
    ...
    low_noise_pred.append(yTest[ind, 0:4])
  elif noise > 0.3 and noise <=0.6:
    med_noise_act.append(y_test[ind,0:4])
    med_noise_pred.append(yTest[ind, 0:4])
  else:
    high_noise_act.append(y_test[ind,0:4])
    high_noise_pred.append(yTest[ind, 0:4])
